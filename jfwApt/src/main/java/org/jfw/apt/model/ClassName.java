...
public final class ClassName extends TypeName {
	...
	private ClassName(String packageName, List<String> names) {
		...
		if (this.packageName != null) {
			this.keyword = packageName + "." + Util.join(".", names);
		} else {
			this.keyword = Util.join(".", names);
		}
	}
	...
	public static ClassName get(TypeElement element) {
		...
		String pn = Util.emptyToNull(getPackage(element).getQualifiedName().toString());
		ClassName result = new ClassName(pn, names);
	...
	public boolean canInstance() {
		if (this.refClass != null) {
			...
			if (java.lang.reflect.Modifier.isAbstract(m) || java.lang.reflect.Modifier.isFinal(m)
			...
			if (cn == null)
				return false;
			if (!java.lang.reflect.Modifier.isPublic(cn.getModifiers()))
				return false;
			return true;
		} else if (this.refElement != null) {
			Set<javax.lang.model.element.Modifier> ms = refElement.getModifiers();
			if (refElement.getKind() != ElementKind.CLASS)
				return false;
			if (ms.contains(Modifier.ABSTRACT) || ms.contains(Modifier.FINAL))
				return false;
			if (!ms.contains(Modifier.PUBLIC))
				return false;
			...
			for (Element e : this.refElement.getEnclosedElements()) {
				...
				if (ee.getModifiers().contains(Modifier.PRIVATE))
					continue;
				if (ee.getModifiers().contains(Modifier.PROTECTED))
					continue;
				if (ee.getParameters() == null || ee.getParameters().size() == 0)
					return true;
			}
			return !existsCo;
		}

	...
	protected void doAfterWhereForFirstPage() {
		if (this.dynamic) {
			if (this.wherePart.hasStaticPart()) {
				...
				cw.bL("if(").w(this.whereSqlName).el(".length()>0){").l("sql.append(\" AND ROWNUM <=\").append(")
						.w(this.pageSizeName).el(").append(\");").l("}else{")
						.bL("sql.append(\" WHERE ROWNUM <=\").append(").w(this.pageSizeName).el(");").l("}");
			}
		} else {

...
public abstract class BaseHandler {
	...
	public void initAfter() throws AptException {
		...
		if (!tn.getClass().equals(ClassName.class))
			throw new AptException(ref, "unsupported class with " + annoName);
	...
	private void check(org.jfw.apt.orm.core.model.CalcColumn col) throws AptException {
		for (org.jfw.apt.orm.core.model.CalcColumn c : this.entry.getColumns()) {
			if (col.getJavaName().equals(c.getJavaName()))
				throw new AptException(ref, "class[" + this.ref.getQualifiedName().toString() + "] exists many attr["
						+ col.getJavaName() + "]");
		}
	}
	...
	protected void handleColumn() throws AptException {
		...
		for (Element ele : eles) {
			org.jfw.apt.orm.annotation.entry.Column col = ele
					.getAnnotation(org.jfw.apt.orm.annotation.entry.Column.class);
			if (col == null)
				continue;

			MethodEntry me = MethodEntry.build((ExecutableElement) ele);
			String javaType = ColumnHandlerFactory.supportedType(col,ele);
			String javaName = null;

			if (me.isGetter()) {
				if (!javaType.equals(me.getReturnType()))
					throw new AptException(ele,
							"invalid return type with @" + org.jfw.apt.orm.annotation.entry.Column.class.getName());
				...
				if (!javaType.equals(me.getParams().get(0).getTypeName()))
					throw new AptException(ele,
							"invalid param type with @" + org.jfw.apt.orm.annotation.entry.Column.class.getName());
			...
			if (de.equals(DE.invalid_de)) {
				...
				org.jfw.apt.orm.core.model.CalcColumn mc = ColumnFactory.buildColumn(ColumnHandlerFactory.getHandlerClass(col, ele), col.nullable(),
						javaType, genDbName(javaName, col.name()), javaName, col.queryable(), col.dbType(),
						Util.emptyToNull(col.fixSqlValueWithInsert()), Util.emptyToNull(col.fixSqlValueWithUpdate()), col.insertable(), col.renewable());
				this.check(mc);
	...
	protected void handleCalcColumn() throws AptException {
		List<? extends Element> eles = ref.getEnclosedElements();
		for (Element ele : eles) {
			org.jfw.apt.orm.annotation.entry.CalcColumn col = ele
					.getAnnotation(org.jfw.apt.orm.annotation.entry.CalcColumn.class);
			if (col == null)
				continue;
			...
			DE de = col.value();
			if (de.equals(DE.invalid_de)) {
				if (null == ColumnHandlerFactory.get(ColumnHandlerFactory.getHandlerClass(col, ele)))
					throw new AptException(ele, "invalid handlerClass with @" + org.jfw.apt.orm.annotation.entry.Column.class.getName());

				org.jfw.apt.orm.core.model.CalcColumn mc = ColumnFactory.buildCalcColumn(ColumnHandlerFactory.getHandlerClass(col,ele),
						col.nullable(), javaType, genDbName(javaName, col.column()), javaName);
				this.check(mc);
				this.entry.getColumns().add(mc);
			} else {
				...
				org.jfw.apt.orm.core.model.CalcColumn mc = ColumnFactory.buildCalcColumn(de.getHandlerClass(),
						de.isNullable(), javaType, genDbName(javaName, col.column()), javaName);
				this.check(mc);
				this.entry.getColumns().add(mc);
			}
		}
